...
/* ── Chat (Web UI answer path: RAG-grounded generation, POST /api/chat SSE) ── */

/** Callbacks for one streamed chat turn. Events arrive in order: sources → chunk* → done|error. */
export interface ChatStreamHandlers {
  /** The RAG grounding for this turn (same shape as search results; may be empty). */
  onSources?: (sources: SearchResult[]) => void;
  /** One answer text delta. */
  onChunk?: (text: string) => void;
  /** Finished successfully; carries the conversation id to continue the thread. */
  onDone?: (conversationId: string) => void;
  /** Generation failed after sources were sent — callers can degrade to showing raw excerpts. */
  onError?: (message: string) => void;
}

/**
 * Streams one chat turn from `POST /api/chat` (text/event-stream over fetch, so POST bodies
 * and AbortController both work). Resolves when the stream ends; per-event data goes through
 * `handlers`. Throws on HTTP-level failures or when `signal` aborts.
 */
export async function chatStream(
  ...
  const res = await fetch("/api/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message, conversationId, web }),
    signal,
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({ error: res.statusText }));
  ...
  if (!res.body) throw new Error("Chat stream unavailable in this browser");
...
/** Parses one SSE frame (`event:`/`data:` lines) and routes it to the matching handler. */
function dispatchSseEvent(raw: string, handlers: ChatStreamHandlers) {
  ...
  try {
    ...
    return; // unparseable frame — skip rather than kill the stream
  }
  switch (event) {
    case "sources":
      handlers.onSources?.(Array.isArray(data) ? (data as SearchResult[]) : []);
      break;
    case "chunk":
      if (typeof data?.text === "string") handlers.onChunk?.(data.text);
      break;
    case "done":
      handlers.onDone?.(typeof data?.conversationId === "string" ? data.conversationId : "");
      break;
    case "error":
      handlers.onError?.(typeof data?.message === "string" ? data.message : "Unknown chat error");
      break;
  }
}

/** Credential status for the chat answer path (shape only — the server never returns secrets). */
export interface ChatCredentialStatus {
  ok: boolean;
  configured: boolean;
  source: "none" | "env" | "runtime";
  validated: boolean;
  message?: string;
}

export async function getChatCredentialStatus(): Promise<ChatCredentialStatus> {
  return json<ChatCredentialStatus>(await fetch("/api/chat/credentials"));
}

/**
 * Pastes signed-in Copilot credentials (accessToken and/or raw Cookie header) and validates
 * them live against the chat socket. In-memory only on the server — nothing is persisted.
 */
export async function connectChatCredentials(input: {
  accessToken?: string;
  identityType?: string;
  cookies?: string;
}): Promise<ChatCredentialStatus> {
  return json<ChatCredentialStatus>(
    await fetch("/api/chat/credentials", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function clearChatCredentials(): Promise<ChatCredentialStatus> {
  return json<ChatCredentialStatus>(
    await fetch("/api/chat/credentials", { method: "DELETE" }),
  );
}
