...
- **Chat** — a persistent conversation thread over one input box. History is kept **client-side only**
  ...
  - **Plain-text messages** → RAG retrieval (§5.6): cited excerpts with source links appear as the next
    turn. No answer text is generated by the server — the excerpts are the response, exactly as in the
    MCP context path.
