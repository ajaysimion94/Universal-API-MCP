...
```
    ...
    ┌─────────────────────────────────┐   ┌───────────────────────────────┐
    │ GitHub Copilot CLI              │   │ Universal Search  (#keywords) │
...
The first-party **Web UI** (§5.8) drives the same two paths through REST controllers: plain keywords in
the universal search box take the context path; `#keyword` invocations take the action path. The UI is a
channel over the same services, not a second implementation (§13).
...
- **Universal Search** — one input box for everything:
  - **Plain keywords** → RAG retrieval: cited excerpts with source links (§5.6), plus live-tool results
    where a read tool is registered for the matched connection.
