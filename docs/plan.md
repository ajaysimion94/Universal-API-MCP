...
connectors console (upload a Postman collection → tools named `{app}_{request-name}`), and a universal
search page where plain keywords run RAG search and `#keyword` invocations run tools deterministically
(e.g. `#todo_app_create_todo "Need to meet chairman"`).
...
+ reranking, Caffeine cache, Postgres-outbox eventing, pgvector — all running as native processes, **no
Docker or virtualization for now**); **Microsoft Teams** (protected-API) and other proprietary
...
- [ ] CI pipeline: build + unit tests + golden-set regression gate on every change (native PostgreSQL
  service on the runner — no Docker)
...
up → durable event queue (Postgres outbox) → File Manager → ACL-tagged, embedded chunks appear in pgvector
within minutes → an AI client requests context for a query → hybrid search (vector + lexical, RRF-merged)
...
- [ ] Cross-encoder reranker **in-process** (e.g. bge-reranker exported to ONNX, same ONNX Runtime setup):
...
- [ ] Confluence connector: Cloud + Server/DC detection, CQL delta polling, webhook registration, backfill
- [ ] Jira connector: Cloud + Server/DC detection, JQL delta polling, webhook registration, backfill
- [ ] **SharePoint connector** (Microsoft Graph — standard permissions, no protected-API approval):
...
- [ ] **Durable event queue** between webhook receipt and processing: PostgreSQL outbox/event table
  (`FOR UPDATE SKIP LOCKED` workers; replay from event id; dead-letter status column; 3-second SLA on
  webhook ack). Kafka (KRaft, native install) is the documented scale-up path if throughput demands it
...
- [ ] **Web UI slice 1 — Universal Search (RAG):** search page returning cited excerpts + source links;
  React + TypeScript SPA built into the JAR's static resources (no separate web server)
...
E2E test checklist:
- [ ] Create a Confluence page → ACL-tagged chunks present in pgvector within the sync SLA (WireMock'd
  Cloud API → real event queue → real pgvector); same test passes against Server/DC (auto-detected,
  polling)
- [ ] Create and edit a Jira issue → chunks appear, then update in place
...
- [ ] **ACL capture asserted:** SharePoint site permission groups and item-level overrides land as the
  expected tags on the chunks (enforcement tests run in Phase 6)
- [ ] Delete the source page/item → its chunks are purged from the vector store
- [ ] AI client requests context for a golden-set question → top result is the expected document; P@1 and
  MRR match baseline
- [ ] Exact-match query (ticket ID, acronym) retrieves correctly via the lexical leg
...
- [ ] Kill ingestion workers mid-burst, restart → no events lost (queue replay verified)
...
- [ ] Golden-set retrieval baseline recorded and wired as regression gate for all later phases
...
- [ ] Disaster recovery: documented RPO/RTO; PostgreSQL + pgvector point-in-time recovery; object
  storage replication
- [ ] Backup strategy: automated backups of PostgreSQL (chunks, audit, workflow state, connections);
  tested restore drill (Keycloak realm backup joins in Phase 6)
