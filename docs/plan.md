...
- [x] **Web UI slice 1 — Chat (RAG):** turn-based chat page — plain-text messages return cited excerpts
  + source links (§5.6), no answer text generated by the server; `#`/`@` tool invocations unaffected,
  still the pure deterministic action path. Conversation history is client-side only (`localStorage`),
  capped, clearable, so prior turns stay visible instead of being replaced. React + TypeScript SPA built
  into the JAR's static resources (no separate web server). (LLM-based answer synthesis was prototyped
  and verified working end-to-end, then explicitly declined — see `DECISIONS.md` 2026-07-25.)
